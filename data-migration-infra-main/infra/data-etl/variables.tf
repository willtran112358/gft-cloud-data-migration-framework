...
variable "sns_event_arn" {
  type        = string
  description = "arn of sns event"
}
...
variable "s3_source_files_dirs" {
  description = "list of names for s3 source files directories and crawler names"
  type        = set(string)
  default     = ["customer", "account", "posting", "loan", "deposit"]
}

############# Crawler variables #############
variable "csv-raw-crawler-tags" {
  type        = map(string)
  description = "crawler tags group"
}

variable "global-reconciliation-crawler-tags" {
  type        = map(string)
  description = "tags group for global reconciliator crawlers"
}

############# Glue Job variables #############

variable "create_balance_calculator_job" {
  description = "Flag to create the resource"
  type        = bool
  default     = false
}
...
variable "worker_type" {
  ...
  validation {
    ...
    error_message = "Worker type must be G.1X, G.2X, G.4X, G.8X."
  }
  default = "G.1X"
}
...
variable "number_of_workers" {
  type        = number
  description = "number of workers, minimun is 2"
  default     = 2
}
...
variable "data_migration_namespace" {
  type        = string
  description = "data migration namespace"
  default     = "dataloader"
}

############# Lambda Reconciliation variables #############
variable "reconciliator_failure_rate" {
  type        = string
  description = "Failure acceptance percentage in reconciliations"
  default     = "0.5"
}

############# variables needed by the step functions #############
variable "global_reconciliation_image" {
  description = "ECR Image PATH form global reconciliator docker image"
  type        = string
}

variable "sf_deploy_job_producer_customer_image" {
  description = "ECR Image PATH form sf deploy job producer customer docker image"
  type        = string
}

variable "sf_deploy_job_producer_account_image" {
  description = "ECR Image PATH form sf deploy job producer account docker image"
  type        = string
}

variable "sf_deploy_job_producer_posting_image" {
  description = "ECR Image PATH form sf deploy job producer posting docker image"
  type        = string
}
...
############# variables needed by Lambda functions #############
variable "aws_lambda_pandas_layer" {
  description = "aws lambda layer for pandas"
  type        = string
}
...
variable "input_database_host" {
  description = "Source data postgresql host"
  type        = string
}


variable "input_database_port" {
  description = "Source data postgresql port"
  type        = number
}


variable "input_database_user" {
  description = "Source data postgresql user"
  type        = string
}


variable "input_database_secret_name" {
  description = "Source data postgresql secret name"
  type        = string
}

variable "input_database_db_name" {
  description = "Source data postgresql database name"
  type        = string
}

variable "input_database_schema_name" {
  description = "Source data postgresql schema name"
  type        = string
}


variable "postgres_to_s3_output_bucket" {
  description = "Postgres to S3 migration ouput bucket"
  type        = string
}